...
class ThemexWoo {
	...
	public static function init() {		
		...
		if(self::isActive()) {
			...
			add_action( 'wp_enqueue_scripts', array(__CLASS__,'filterStyles'), 99 );
	...
	public static function filterPages() {
		...
		if(self::isActive()) {
			if(!is_user_logged_in() && is_account_page()) {
				wp_redirect(ThemexUser::$data['register_page_url']);
				exit;
			}
	...
	public static function filterStyles() {
	...
	public static function getPrice($ID=0) {
		if(self::isActive()) {
			$product=new WC_Product_Simple( intval($ID) );			
