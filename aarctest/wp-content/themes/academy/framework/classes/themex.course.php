...
class ThemexCourse {
	...
	public static function saveCourse($ID) {
		...
		if(isset($_POST['add_user']) && isset($_POST['add_user_id'])) {
			if($post->post_type=='course') {
				...
				$courses = self::getPlanCourses($ID);
				foreach($courses as $idx=>$val){
		      self::addUser(intval($val), intval($_POST['add_user_id']));
		    }
			}
			...
			if($post->post_type=='course') {
				...
				$courses = self::getPlanCourses($ID);
				foreach($courses as $idx=>$val){
		      self::removeUser(intval($val), intval($_POST['remove_user_id']));
		    }				
			}
		}
	}
			...
			switch($_POST['course_action']) {
				...
				case 'register':
						...
						for($i=0; $i <= $course_count; $i++ ){
						  self::addUser(intval($_POST['course_id'][$i]));
						}						
				break;
				
				case 'deregister':
						self::unsubscribeUser(intval($_POST['plan_id']));
						$courses = $_POST['course_id'];
						$course_count = count($_POST['course_id']);
						for($i=0; $i <= $course_count; $i++ ){
						  self::removeUser(intval($_POST['course_id'][$i]), intval($_POST['user_id']));
						}						
				break;
	...
	public static function getPlanPrice($ID=0) {
		...
		if(ThemexWoo::isActive()) {
			...
			if(get_post_meta($ID, '_plan_period', true) == 'none'){
			  $period=get_post_meta($ID, '_plan_period', true);
			} else {
  ...
  function getPlanCourses($id) {
	...
	public static function getSubscriptions($ID=null) {
		...
		if(isset(self::$data['course']['subscriptions']) && !isset($_POST['course_action'])) {
			...
			foreach($plans as $plan_ID) {
				...
				if($users[$ID] == 'none'){
				  $subscriptions[$plan_ID]=$users[$ID];
				} else {
	...
	public static function getSubscriptionTime() {
		...
		if( !empty($subscriptions) ) {
		  ...
		  if ( in_array('none',$subscriptions) ) {

//    This adds a subscription message for users with a subscription ID set to none		    
//		    $plan=key($subscriptions);			
//		    $message='"'.get_the_title($plan).'" '.__('subscription does not expire', 'academy').' ';
	...
	//Detect plan subscriber
	public static function isSubscribed($id) {
	  $planUsers = ThemexCore::parseMeta($id, 'plan', 'users');
//	   = get_post_meta($id,'_plan_users');
		if(is_user_logged_in() && array_key_exists(get_current_user_id(), $planUsers)) {
			return true;
		}
	...
	//Get post-requisite lesson URL, i.e., lesson URL with a prerequisite of the current post. ONLY RETURNS TOP OF ARRAY
	public static function getLessonPostreqURL($ID=0) {
	  $args = array(
	   'post_type' => 'lesson',
	   'meta_key' => '_lesson_prerequisite',
	   'meta_value' => $ID
	  );
		$postreqs = get_posts($args);
    
    $lessons = array();
 		foreach ($postreqs as $postreq ){
 		  $lessons[] = get_permalink($postreq->ID); 
 		}
	...
	public static function subscribeUser($ID=0, $user=null) {
		...
		if (get_post_meta($ID, '_plan_period', true) == 'none') {
		  ...
		  if (intval(get_post_meta($ID, '_plan_period', true)) == 0) {
  		  $users[$user]=intval(get_post_meta($ID, '_plan_period', true));
  		} else {
