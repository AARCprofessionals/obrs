...
function mtq_update_status(mtqid){
	...
	$score = mtq_score_percent[mtqid].toFixed(0);
	
	if(mtq_score_percent[mtqid].toFixed(0) == '100%') {
		jQuery('a.submit-button').show();
		jQuery('a.submit-button').click();
	}
}
...
function mtq_button_click (q,a,mtqid)
{
	...
	if (mtq_autoadvance[mtqid]) {
		if( is_answered || question_correct ){
			setTimeout("mtq_next_nav("+mtqid+")",10000); // changed from 1000ms to 10000ms !! Also changed in script.min.js !!
		}
	}
...
function mtq_score_blindly(mtqid){ // This assumes that there was only one attempt for each problem and ignores number of attempts. 
	...
	jQuery(div.hidden.score).html(mtq_score_percent[mtqid].toFixed(0));
	if(mtq_score_percent[mtqid].toFixed(0) == '100%') {
		jQuery('a.submit-button').show();
		jQuery('a.submit-button').click();
	}
