    ...
    function npcms_membership_login_shortcode_body( $attr, $content = null ) {
        ...
        if (isset($_POST['Username']) && isset($_POST['Password'])) {
            ...
            if (!$response) {
				...
				//Auto-generate WordPress User ID
				$user_password = $_POST['Password'];
				if ( username_exists( $user_name ) == false ) {
					wp_create_user( $user_name, $_POST['Password'], $user_name.'@aarc.org' );
					$success = '<p>You have successfully registered your account. You can now login. Please <a href="?npcms_logout=true">Click here</a> to sign in.</p>';
					return __( $success, 'npcms_membership');
				...
				$user_info = get_userdatabylogin($user_name);
				$userid = $user_info->ID;
				//Query Metadata true: return result as string, false: return result as array
				$values = get_post_meta($course_number, '_plan_users', true); 
				if (strpos($values,'&'.$nameid.'=none') == false){
						$meta_plan_users = $values . '&'.$userid.'=none';
						update_post_meta($course_number, '_plan_users', $meta_plan_users);
				}
				
				$regular = array(3653, 3643, 222, 2172, 2222. 2223, 2470, 2484, 2490, 2509, 2511, 3666, 2513, 3670, 3666);
				foreach$regular as $regular) {
				  if($content = getData($regular)) {
					$values = get_post_meta($content, '_course_users', true);
						if (strpos($values,'&'.$nameid.'=none') == false){
							$meta_plan_users = $values . '&'.$userid.';
							update_post_meta($course_number, '_course_users', $meta_plan_users);
					
						}
				  }
				}
					$california = array(2513, 3670); 

				
			     */
                //$redirectScript = '<script>window.location = "'.get_permalink($id).'";</script>';
				$redirectScript = '<script>window.location = "plan/unlimited/";</script>';
                /*
				if ($urlToDirectTo)
                {
