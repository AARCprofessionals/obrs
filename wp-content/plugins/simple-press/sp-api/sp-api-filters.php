...
Simple:Press
Filters
$LastChangedDate: 2014-04-12 13:40:59 -0700 (Sat, 12 Apr 2014) $
$Rev: 11287 $
*/
...
function sp_filter_save_escape($content) {
    ...
    # handle wp esc_sql() double slashing our return chars
    $search = array("\\n","\\r");
    $replace = array("\n","\r");
...
function sp_filter_content_edit($content) {
	...
	# 4: Format the paragraphs (p and br onlt Richtext)
	...
	# 5: Parse post into appropriate editor format
	$content = sp_filter_edit_parser($content, $spGlobals['editor']);

	# 6: Turn off shortcode processing to keep from messing up their display
...
function sp_format_display_image($match) {
	...
	if ((int) $width[1] > (int)$thumb) { # is width > thumb size
		...
		$anchor = true;
	} elseif (!empty($width[1]) && $width > 0) { # width is smaller than thumb, so use the width {
		$thiswidth = 'width="'.$width[1].'" ';
		$mclass = '';
		$anchor = false;
	} else { # couldn't determine width, so don't output it
		$thiswidth = '';
		$mclass = '';
		$anchor = false;
	}
	...
	if ($anchor) {
		if (!empty($width) && $width[1] ? $w = $width[1] : $w = 'auto');
		if (!empty($height) && $height[1] ? $h = $height[1] : $h = 'auto');
		# Use popup or not?
