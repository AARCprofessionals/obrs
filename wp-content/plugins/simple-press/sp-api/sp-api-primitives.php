...
$LastChangedDate: 2014-03-17 10:57:49 -0700 (Mon, 17 Mar 2014) $
$Rev: 11173 $
*/
...
#	Propose 1 - 50 reserved for core use
#	Propose 51 - to be used by plugins
#
# ------------------------------------------------------------------
# sp_add_activity()
#
...
# ------------------------------------------------------------------
function sp_add_activity($userid, $type, $value, $doCheck = true) {
	if(empty($userid) || empty($type) || empty($value)) return false;

	# maybe need to check record does not already exist
	if($doCheck) {
		$sql = 	'SELECT id FROM '.SFUSERACTIVITY.
				" WHERE user_id=$userid AND type_id=$type AND item_id=$value";
		$check = spdb_select('var', $sql);
		if($check) return true;
	}

	# so insert new record
	$sql =  'INSERT INTO '.SFUSERACTIVITY.
			"(user_id, type_id, item_id)
			VALUES
			($userid, $type, $value)";
...
# ------------------------------------------------------------------
function sp_get_activity($userid, $type, $value) {
	if(empty($userid) || empty($type) || empty($value)) return false;
...
function sp_get_all_activity($userid, $type) {
	...
	$sql = 	'SELECT item_id FROM '.SFUSERACTIVITY.
			" WHERE user_id=$userid AND type_id=$type";
...
#
# Version: 5.4.2
# Gets couimt of records from the user activity table of specific type
#	$userid		ID of the user
#	$type:		The type of the activity record
#
...
function sp_get_activity_value($type, $value) {
	...
	$sql = 	'SELECT user_id FROM '.SFUSERACTIVITY.
...
function sp_delete_activity($userid, $type, $value) {
    ...
    $sql = 'DELETE FROM '.SFUSERACTIVITY.
    	   " WHERE user_id=$userid AND type_id=$type AND item_id=$value";
	return spdb_query($sql);
}
