...
$LastChangedDate: 2014-02-16 08:54:17 -0800 (Sun, 16 Feb 2014) $
$Rev: 11078 $
*/
...
jQuery(document).ready(function() {
	jQuery('#rolerow-<?php echo $role_id; ?>').addClass('inForm');
...
<?php
	...
	<form action="<?php echo $ahahURL; ?>" method="post" id="sfroleedit<?php echo $role->role_id; ?>" name="sfroleedit<?php echo $role->role_id; ?>">
		...
		<input type="button" class="button-primary" onclick="javascript:jQuery('#perm-<?php echo $role->role_id; ?>').html('');jQuery('#rolerow-<?php echo $role_id; ?>').removeClass('inForm');" id="sfpermedit<?php echo $role->role_id; ?>" name="editpermcancel<?php echo $role->role_id; ?>" value="<?php spa_etext('Cancel'); ?>" />
		</div>
		</form>
