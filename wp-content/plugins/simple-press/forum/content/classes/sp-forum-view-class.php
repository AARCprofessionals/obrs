...
Simple:Press
Forum View Class
$LastChangedDate: 2014-04-12 07:42:52 -0700 (Sat, 12 Apr 2014) $
$Rev: 11275 $
*/
...
class spForumView {
	...
	function sp_forumview_query($forumid=0, $cPage=1) {
		...
		$spdb = new spdbComplex;
			$spdb->table	= SFTOPICS;
			$spdb->fields	= SFTOPICS.'.forum_id, forum_slug, forum_name, forum_status, group_id, topic_count, forum_icon, topic_icon, topic_icon_new, topic_icon_locked, forum_desc, forum_rss,
	...
	function sp_forumview_subforums_query($subs, $top = false) {
		...
		if (!empty($spGlobals['disabled_forums'])) {
			foreach ($subs as $thisSub) {
				if (!in_array($thisSub, $spGlobals['disabled_forums'])) $s[] = $thisSub;
			}
		} else {
			$s = $subs;
		}
		...
		$spdb = new spdbComplex;
			$spdb->table	= SFFORUMS;
			$spdb->fields	= 'forum_id, forum_name, forum_slug, forum_desc, forum_seq, forum_status, forum_disabled, forum_icon, forum_icon_new, topic_icon, topic_icon_new, topic_icon_locked, forum_rss_private,
