...
$LastChangedDate: 2014-04-08 02:31:26 -0700 (Tue, 08 Apr 2014) $
$Rev: 11255 $
*/
...
function sp_TopicIndexIcon($args='') {
	...
	} else if ($spThisTopic->topic_status && !empty($spThisForum->topic_icon_locked)) {
...
function sp_TopicIndexStatusIcons($args='', $toolTipLock='', $toolTipPin='', $toolTipPost='', $toolTipDenied='') {
	...
	if (!$spThisTopic->reply_topics && !empty($toolTipDenied)) {
		$icon = SPTHEMEICONSURL.sanitize_file_name($iconDenied);
		$out.= "<img class='$iconClass vtip' src='$icon' title='$toolTipDenied' alt='' />\n";
	}
